...
逻辑撤销。服务端必须关闭现有 WS、退款该设备未 started 的 held command，并拒绝后续事件。
...
重复提交同一 Idempotency-Key 返回同一 command 和当前状态；不得再次冻结余额。

#### GET `/commands/{command_id}`

返回 command、turn、billing 和 error 状态，不返回原始 prompt。

#### POST `/commands/{command_id}/cancel`

- `reserved/dispatched/accepted`：取消并退款。
- `started`：映射为 app-server `turn/interrupt`，但费用已结算，不退款。
- terminal state：幂等返回现状。

### 3.5 收费记录

#### GET `/charges?page=1&page_size=20`

返回用户自己的协同账单：command、设备名称、thread 标题快照、金额、币种、held/settled/refunded、时间和原因。不返回 prompt。
...
| `command.dispatched` | command_id + thread_id + input + expires_at | 下发已冻结任务 |
| `command.cancel_requested` | command_id + optional turn_id | 取消/打断 |
| `approval.decision` | approval_id + decision | 后续移动审批 |
| `server.shutdown` | retry_after_seconds | 优雅迁移连接 |

PC 收到 command 后，先发送 `command.received`，再进行 thread 校验。收包 ACK 不等于 accepted，也不能触发结算。
...
| `command.accepted` | command_id | thread 可尝试写入 |
| `command.started` | command_id + turn_id | 触发结算 |
| `command.failed_to_start` | command_id + error_code | 触发退款 |
| `codex.item.event` | command_id/thread/turn/item | 流式 item |
| `codex.turn.completed` | command_id + status | terminal result |
| `codex.approval.requested` | approval DTO | PC 审批中心/移动状态 |

服务端必须验证事件对应资源属于该 WS 的 device/user，且状态转换合法；错误或重放事件只记审计，不重复结算。
...
| `command.status_changed` | reserved/started/refunded 等 |
| `codex.item.event` | 当前订阅 thread 的流式 UI |
| `codex.turn.completed` | turn 收尾 |
| `codex.approval.requested` | MVP 显示“等待 PC 审批” |
...
`command.started` 只能在 `turn/start` 返回成功之后发送。不能在 `thread/resume` 成功时结算。
...
1. Companion 把请求显示在 PC 审批中心。
2. 向 Mobile 发只读摘要和 `waiting_approval` 状态。
3. 只有 PC 用户明确选择后，Companion 才向 app-server 返回 decision。
4. 不将默认策略设为 `never`，也不为了移动协同开启 danger full access。

后续移动审批必须显示 command、cwd、diff/权限和可用 decisions；服务端不能替用户改写 decision。
...
| 403 | `COLLAB_DEVICE_FORBIDDEN` | 返回设备列表 | 无/退款 |
| 403 | `COLLAB_THREAD_FORBIDDEN` | 刷新列表 | 无/退款 |
| 409 | `COLLAB_DEVICE_OFFLINE` | 显示离线 | 无 |
| 409 | `COLLAB_THREAD_BUSY_EXTERNAL` | 提示关闭原 Codex 或 fork | 退款 |
| 409 | `COLLAB_TURN_IN_PROGRESS` | 排队/稍后重试 | 保持 held 或退款，策略固定 |
| 409 | `COLLAB_INSUFFICIENT_BALANCE` | 展示余额和充值入口 | 无 |
| 410 | `COLLAB_COMMAND_EXPIRED` | 可重新发送 | 退款 |
| 422 | `COLLAB_CODEX_INCOMPATIBLE` | 提示升级 Codex/Companion | 退款 |
| 429 | `COLLAB_RATE_LIMITED` | 倒计时重试 | 无 |
| 502 | `COLLAB_CODEX_START_FAILED` | 显示 PC 诊断 | 退款 |
| 503 | `COLLAB_RELAY_UNAVAILABLE` | 稍后重试 | 无/退款 |
